      ...
      <div className="px-6 sm:px-10 py-16 sm:py-24">
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          ...
          <div className="flex flex-col items-center text-center">
            <div className="w-12 h-12 rounded-full flex items-center justify-center mb-4" style={{ backgroundColor: '#F4F3F0' }}>
              <svg className="w-6 h-6 text-brand-bronze" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <p className="text-xs uppercase tracking-widest text-brand-hint font-medium mb-3">Enquiry received</p>
          ...
          <ol className="mt-10 space-y-4">
            {NEXT_STEPS.map((step, i) => (
              <li key={i} className="flex items-start gap-3">
                <span
                  className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-semibold text-white mt-0.5"
          ...
          <div className="mt-12 pt-8 border-t border-brand-border text-center">
            ...
            <Link
              href="/projects"
              className="inline-flex items-center justify-center min-h-[48px] px-6 rounded-lg text-sm font-semibold text-white transition-opacity hover:opacity-90"
              style={{ backgroundColor: '#A0784A' }}
            >
              Browse projects
            </Link>
