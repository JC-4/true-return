...
export default function GallerySlider({ images, onOpenLightbox, variant = 'section' }: {
  ...
  return (
    <section id="gallery" className={variant === 'plain' ? '' : 'py-16 border-t border-brand-border'}>
        {variant === 'section' && (
          <p className="text-sm font-medium text-brand-muted mb-6">Gallery</p>
        )}
          ...
          {images.length > 1 && (
            <div className="absolute bottom-4 right-4 flex items-center gap-2">
              <button
                ...
                style={{ backgroundColor: "rgb(var(--tl-rgb) / 0.15)", backdropFilter: 'blur(4px)', color: 'var(--tl)', border: "0.5px solid rgb(var(--tl-rgb) / 0.25)" }}
              >
            ...
            {images.map((_, i) => (
              <button
                ...
                style={{
                  ...
                  height: 6,
                  borderRadius: 9999,
                  backgroundColor: i === current ? 'var(--c-accent)' : 'var(--c-hint-faint)',
                  border: 'none',
                  padding: 0,
...
export function Lightbox({
  ...
  return (
    ...
    >
      ...
      {/* Counter */}
      <div className="absolute top-5 left-1/2 -translate-x-1/2 text-brand-tl/60 text-sm tabular-nums">
      ...
      {images.length > 1 && (
        <button
          ...
          className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center text-brand-tl/70 hover:text-brand-tl"
      ...
      {images.length > 1 && (
        <button
          ...
          className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center text-brand-tl/70 hover:text-brand-tl"
